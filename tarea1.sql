...
#Muestra el listado aplicando JOIN y subconsultas.
SELECT c.id, t.id, c.company_name, c.country
FROM company c
INNER JOIN transaction t ON c.id = t.company_id
...
SELECT c.company_name, c.country, DATE(t.timestamp) AS fecha, t.amount
FROM company c
INNER JOIN transaction t ON c.id = t.company_id
...
SELECT c.company_name, COUNT(t.id) as cant_transacciones, CASE 
           WHEN COUNT(t.id) > 400 THEN 'Sí' #si el total de t.id es mayor a 400. escribir Sí. 
           ELSE 'No' # en caso de que no se compla la condición anterior, escribir No.
       END AS mas_400_transacciones # con CASE se agrega una columna que específica si hay más de 400 transacciones
FROM transaction t
